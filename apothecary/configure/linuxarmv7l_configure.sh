#!/usr/bin/env bash
# Raspberry Pi OS 32-bit ARMv7 (ARCH=armv7l).
# Official CI uses host GCC 10 + a bookworm armhf sysroot.
...
RPI_ARCH_FLAGS="${RPI_ARCH_FLAGS:--march=armv7-a -mtune=cortex-a7 -mfpu=neon-vfpv4 -mfloat-abi=hard}"

if type rpi_is_native >/dev/null 2>&1 && rpi_is_native && [[ "$HOST_ARCH" == "armv7l" ]]; then
    CROSSCOMPILE=0
...
if [[ "$CROSSCOMPILE" -eq 0 ]]; then
    ...
    export ROOTFS="${SYSROOT:-/opt/rpi-armv7l-sysroot}"
    export SYSROOT="${SYSROOT:-/opt/rpi-armv7l-sysroot}"
    export TOOLCHAIN_ROOT="${TOOLCHAIN_ROOT:-/usr}"
...
echo "--------------------"
echo "openFrameworks apothecary Raspberry Pi armv7l"
