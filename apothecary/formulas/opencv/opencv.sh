...
function build() {
    ...
    if [[ "$TYPE" =~ ^(osx|ios|tvos|xros|catos|watchos)$ ]]; then
        ...
        if [[ "$ARCH" =~ ^(arm64ec|arm64)$ ]]; then  # ARM64 on Windows: Use NEON
            EXTRA_DEFS="-DCV_ENABLE_INTRINSICS=ON -DCPU_BASELINE='' -DCPU_DISPATCH='' -DBUILD_opencv_rgbd=OFF -DPNG_ARM_NEON=OFF"
            ...
            EXTRA_DEFS="-DCV_ENABLE_INTRINSICS=ON -DCPU_BASELINE='' -DCPU_DISPATCH='' -DPNG_ARM_NEON=OFF -DPNG_INTEL_SSE=ON"
