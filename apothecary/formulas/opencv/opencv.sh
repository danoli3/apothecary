...
function build() {
    ...
    if [[ "$TYPE" =~ ^(osx|ios|tvos|xros|catos|watchos)$ ]]; then
        ...
        if [[ "$ARCH" =~ ^(arm64ec|arm64)$ ]]; then  # ARM64 on Windows: Use NEON
            EXTRA_DEFS="-DCV_DISABLE_OPTIMIZATION=OFF -DCV_ENABLE_INTRINSICS=OFF -DBUILD_opencv_rgbd=OFF -DPNG_ARM_NEON=on"
            ...
            EXTRA_DEFS="-DCV_DISABLE_OPTIMIZATION=OFF -DCV_ENABLE_INTRINSICS=OFF -DPNG_ARM_NEON=off -DPNG_INTEL_SSE=on" #must use lowercase on for this png
