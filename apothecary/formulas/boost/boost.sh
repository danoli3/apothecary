...
EOF
        ...
        for NAME in $ALL_LIBS; do
            ...
            if [[ "$TYPE" == "ios" ]]; then
                (
                    cd $IOSBUILDDIR/armv7/$NAME-obj
                    ar -x ../$NAME.a
                )
            fi
            (
                cd $IOSBUILDDIR/arm64/$NAME-obj
                ar -x ../$NAME.a
            )
            (
                cd $IOSBUILDDIR/x86_64/$NAME-obj
                ar -x ../$NAME.a
            )
        done
        echo "done"
        ...
        for NAME in $ALL_LIBS; do
            ...
            if [[ "$TYPE" == "ios" ]]; then
                (
                    cd $IOSBUILDDIR/armv7
                    $ARM_DEV_CMD ar crus re-$NAME.a $NAME-obj/*.o
                )
            fi
            (
                cd $IOSBUILDDIR/arm64
                $ARM_DEV_CMD ar crus re-$NAME.a $NAME-obj/*.o
            )
            (
                cd $IOSBUILDDIR/x86_64
                $SIM_DEV_CMD ar crus re-$NAME.a $NAME-obj/*.o
            )
        done
        echo "done"
        echo "---------------"
