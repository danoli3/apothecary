...
while IFS= read -r formula; do
    ...
    if ! grep -Eq '(^|[[:space:]])(downloader|curl|git clone)([[:space:]]|$)' <<< "$active"; then
        continue
    fi
    network_formulas=$((network_formulas + 1))

    if grep -Eq 'verify_sha256|verify_git_commit|commit mismatch' <<< "$active"; then
    ...
    if grep -Eq "SHA(256)?=[[:space:]]*(\"\"|'')" <<< "$active"; then
