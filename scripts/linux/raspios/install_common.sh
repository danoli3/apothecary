#!/usr/bin/env bash
# Install Raspberry Pi *target* headers/libs into the sysroot.
# Required: RPI_SLICE GCC_TRIPLE QEMU_STATIC DEFAULT_SYSROOT NATIVE_UNAME
set -euo pipefail
...
if rpi_is_native && [[ "$(uname -m)" == "$NATIVE_UNAME" ]]; then
    ...
    if [[ ! -f "$(rpi_libc_path "$SYSROOT" "$GCC_TRIPLE")" ]]; then
...
export PKG_CONFIG_LIBDIR="$SYSROOT/usr/lib/${GCC_TRIPLE}/pkgconfig:$SYSROOT/usr/share/pkgconfig"
export PKG_CONFIG_PATH=""
pkg-config --list-all | head -20 || true

echo "=== raspios $RPI_SLICE install complete ==="
